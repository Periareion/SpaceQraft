...
class Camera(Entity):
    ...
    def movement(self, mouse, keyboard):
        ...
        if keyboard.state[pygame.K_w]: self.relative_translation(Q([0, 0, 1])*0.03)
        if keyboard.state[pygame.K_s]: self.relative_translation(Q([0, 0, -1])*0.03)
        if keyboard.state[pygame.K_a]: self.relative_translation(Q([-1, 0, 0])*0.03)
        if keyboard.state[pygame.K_d]: self.relative_translation(Q([1, 0, 0])*0.03)
        if keyboard.state[pygame.K_SPACE]: self.relative_translation(Q([0, -1, 0])*0.03)
        if keyboard.state[pygame.K_LSHIFT]: self.relative_translation(Q([0, 1, 0])*0.03)
