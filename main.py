...
def main(width, height, FPS=60):
    ...
    camera_position = Q([0, 0, 3])
    camera_unit_vectors = graphics.get_camera_unit_vectors(Q([0, 0, -1]), Q([0, 1, 0]))
    ...
    cubes = gm.Group([
        ...
        gm.Cuboid((0.5, 0, 0),(1, 0.5, 0.5),(0.35,0.9,0.25))])

    sphere = gm.Group([gm.Sphere(radius=1)])
    running = True
    while running:

        window.fill([0.2*255, 0.2*255, 0.3*255])
        ...
        clock.tick(FPS)
        fps = clock.get_fps()
        pygame.display.set_caption(f'Qraft (FPS: {fps:.2f})')
